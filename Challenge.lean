/-
  ...
  ── SCOPE ──────────────────────────────────────────────────────────────────
  All 20 theorems listed in `Audit.lean` (ordered by importance).

  ── CONTENTS ───────────────────────────────────────────────────────────────
  §0.  Inline definitions (Mathlib-only reproductions)
  §1.  PrimeSumset — Main results                               (3 theorems)
  §2.  RestrictedGoldbach — Main result                          (1 theorem)
  §3.  RestrictedGoldbach — Tier‑1 analytic gap                  (1 theorem)
  §4.  RestrictedGoldbach — Combinatorial backbone               (2 theorems)
  §5.  PrimeSumset — Additive-energy combinatorics               (5 theorems)
  §6.  RestrictedGoldbach — Set combinatorics                    (3 theorems)
  §7.  RestrictedGoldbach — Tier‑1 stubs / definitions           (4 theorems)
  §8.  PrimeSumset — Basic facts                                 (1 theorem)
...
-- ══════════════════════════════════════════════════════════════════════════
-- §1.  PrimeSumset — Main results  (most important)
--      Sources: MainTheorem.lean, Goldbach.lean
-- ══════════════════════════════════════════════════════════════════════════
...
/- **sumset_card_gt_95** (0‑indexed framing)
   For i > 10^15, |sumset(Aset(pᵢ), Bset(pᵢ))| > 0.95·pᵢ.
...
/- **goldbach_density**
   For i > 10^15, the number of representable N ∈ (pᵢ, 3pᵢ] exceeds 0.95·pᵢ. -/
theorem goldbach_density {i : ℕ} (hi : i > trigger) :
    (95 / 100 : ℝ) * (primeIdx i : ℝ) <
    (#{N ∈ Finset.Ioc ((primeIdx i : ℤ)) (3 * primeIdx i) |
       0 < rAdd (Aset (primeIdx i)) (Bset (primeIdx i)) N} : ℝ) := by
  sorry

/- **goldbach_exception_bound**
   For i > 10^15, the number of even non‑representable N ∈ (pᵢ, 3pᵢ] is below 0.05·pᵢ. -/
theorem goldbach_exception_bound {i : ℕ} (hi : i > trigger) :
    (#{N ∈ Finset.Ioc ((primeIdx i : ℤ)) (3 * primeIdx i) |
       Even N ∧ rAdd (Aset (primeIdx i)) (Bset (primeIdx i)) N = 0} : ℝ) <
    (5 / 100 : ℝ) * (primeIdx i : ℝ) := by
  sorry

end PrimeSumset

-- ══════════════════════════════════════════════════════════════════════════
-- §2.  RestrictedGoldbach — Main result
--      Source: RestrictedGoldbachMain.lean
-- ══════════════════════════════════════════════════════════════════════════
...
/- **sumset_card_gt_95** (strongest achieved constant, 0.95)
...
-- ══════════════════════════════════════════════════════════════════════════
-- §3.  RestrictedGoldbach — Tier‑1 analytic gap
...
-- ══════════════════════════════════════════════════════════════════════════
-- §4.  RestrictedGoldbach — Combinatorial backbone
...
-- ══════════════════════════════════════════════════════════════════════════
-- §5.  PrimeSumset — Additive-energy combinatorics
...
-- ══════════════════════════════════════════════════════════════════════════
-- §6.  RestrictedGoldbach — Set combinatorics
--      Source: RestrictedGoldbachCombinatorics.lean
-- ══════════════════════════════════════════════════════════════════════════
...
-- ══════════════════════════════════════════════════════════════════════════
-- §7.  RestrictedGoldbach — Tier‑1 stubs / definitions
--      Sources: RestrictedGoldbachMajorArc.lean,
--               RestrictedGoldbachDefs.lean
-- ══════════════════════════════════════════════════════════════════════════
...
/- **restricted_major_arc_asymptotic**  (Obligation A, Tier‑1 stub)
   The restricted major‑arc asymptotic product theorem.
   Currently stated as `True`; formalization is a Tier‑1 task. -/
...
/- **three_le_primeIdx_of_trigger**
   Above the trigger, the i‑th prime is at least 3. -/
...
/- **mass_pos**
   The mass M = |A|·|B| is positive once 3 ≤ x. -/
